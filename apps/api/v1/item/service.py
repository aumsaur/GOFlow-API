...
def create_item(request: Request, db: Session = Depends(deps.get_db), *, item: schemas.ItemCreate):
    ...
    return crud.item.create(db=db, obj_in=item, owner_id=user_id)
...
def fetch_item(request: Request, db: Session = Depends(deps.get_db)):
    user_id = request.state.token_sub.get('id')
    owned_item = crud.item.get_by_owner(db=db, owner=user_id)
...
def update_item(request: Request, db: Session = Depends(deps.get_db), *, item_id: str, item: schemas.ItemUpdate):
    db_item = crud.item.get(db=db, id=item_id)
    if db_item is None:
        raise HTTPException(status_code=404, detail="Item not found")
...
def delete_item(request: Request, db: Session = Depends(deps.get_db), *, item_id: str):
    user_id = request.state.token_sub.get('id')
    db_item = crud.item.get(db=db, id=item_id)
    if db_item is None:
        raise HTTPException(status_code=404, detail="Item not found")
    if db_item.owner_id != user_id:  # not owner
        raise HTTPException(
...
@router.get("/fetch/relation", tags=["Debugs"])
def fetch_relation(request: Request, item_id: str, db: Session = Depends(deps.get_db)):
    db_item = crud.item.get(db=db, id=item_id)
    owner_id = request.state.token_sub.get('id')
    if db_item is None:
        raise HTTPException(status_code=404, detail="Item not found")
    if db_item.owner_id == owner_id:  # Have permission to manipulate
        print(db_item.owner_id + ' ; ' + owner_id)
    return db_item.owner
